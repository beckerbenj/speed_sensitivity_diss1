...
# set wd for graphs
setwd("c:/Benjamin_Becker/00_Promotion/20_Presentations/Graphs/3forms")
...
subgrpAnalysis <- function(plotData) {
  plotData %>%
    ...
    summarise('M(cumRT)' = mean(cumRT), 
              ...
              'M(diff)' = mean(estimatedAbil - abil))
}
...
# Bias in ability estimation between testforms and per speed level for high ability persons
# with(plotData, tapply(estimatedAbil - abil, INDEX = list(speed, Booklet), mean))

resultList2 <- lapply(resultList, function(x) {
  x[x$abil > 0.67, ] 
})
#lapply(resultList2, dim)

tableList$bias_mean_all_habil <- averageResults(resultList2, FUN = biasAnalysis, replicationFUN = mean)
tableList$bias_sd_all_habil <- averageResults(resultList, FUN = biasAnalysis, replicationFUN = sd)
...
tableCaptions$mean_all <- "Test Statistics per Test Form and per Speed Group, Averaged Across All Replications."
...
tableCaptions$bias_mean_all_habil <- "Analysis of Bias in Person Parameter Estimation and Differences in Bias Between Booklets for Speed Subgroups, Averaged Across All Replications."

# hotfix variable names table
names(tableList$bias_mean_all) <- c("Speed", "$\\Delta(\\phi_{low}, \\phi_{medium})$", "$\\Delta(\\phi_{low}, \\theta_{phi})$")
names(tableList$bias_mean_all_habil) <- c("Speed", "$\\Delta(\\phi_{low}, \\phi_{medium})$", "$\\Delta(\\phi_{low}, \\theta_{phi})$")

tableList$sd_all$Booklet <- tableList$mean_all$Booklet <- c(rep("low $\\phi$", 4), rep("medium $\\phi$", 4), rep("high $\\phi$", 4))
names(tableList$sd_all) <- names(tableList$mean_all) <- c("Test Form", "$\\zeta_{i}$", "$M(RT_{tot})$", "$SD(RT_{tot})$", "$M(Mis)$", "$SD(Mis)$",
                               "$cor(\\hat{\\theta} \\theta)$", "RMSE", "$M(\\theta_{diff})$")
...
library(latex2exp)
plot_expected_RTs <- function(s2_e, phi, func, lims, title = "", xlabel) {
  zeta1 <- 4 - phi * 1
  zeta2 <- 4 - phi * -1
  # add vline
  mean1 <- zeta1
  ...
  out <- ggplot(data = df, aes(x)) +
    stat_function(fun = func, n = 101, args = list(mean = zeta1, sd = sqrt(s2_e)), size = line_size) +
    stat_function(fun = func, n = 101, args = list(mean = zeta2, sd = sqrt(s2_e)), colour = "darkgrey", size = line_size) + 
    # stat_function(fun = func, n = 101, args = list(mean = zeta2, sd = sqrt(1/alpha)), linetype = 2) + 
    ylab("") + xlab(xlabel) +
    apatheme 
  
    # intercept limits:
  yend_rt <- dnorm(x = mean1, mean = mean1, sd = sqrt(s2_e))
  #yend_rt <- dnorm(x = mean2, mean = mean2, sd = sqrt(s2_e))
  yend_logrt1 <- dlnorm(x = exp(zeta1), mean = zeta1, sd = sqrt(s2_e))
  yend_logrt2 <- dlnorm(x = exp(zeta2), mean = zeta2, sd = sqrt(s2_e))
  
  type <- 2
  if(max(df$x) == 8) out <- out + annotate("label", x=4, y=0.9, label=TeX(title, output = "character"), parse=TRUE, size = 13, fill = "grey") + 
    scale_y_continuous(breaks = NULL, limits = c(0, 1.1)) + 
    geom_segment(aes(x = mean1, y = -Inf, xend = mean1, yend = yend_rt), colour = "black", linetype = type, size = line_size) + 
    geom_segment(aes(x = mean2, y = -Inf, xend = mean2, yend = yend_rt), colour = "darkgrey", linetype = type, size = line_size)
  if(max(df$x) == 320) out <- out + annotate("label", x=175, y=0.032, label=TeX(title, output = "character"), parse=TRUE, size = 13, fill = "grey") +
    scale_y_continuous(breaks = NULL, limits = c(0, 0.037)) +
    geom_segment(aes(x = mean1, y = -Inf, xend = mean1, yend = yend_logrt1), colour = "black", linetype = type, size = line_size) + 
    geom_segment(aes(x = mean2, y = -Inf, xend = mean2, yend = yend_logrt2), colour = "darkgrey", linetype = type, size = line_size)
  out
}


lim_RT <- replicate(4, c(0, 8), simplify = F)
lim_logRT <- replicate(4, c(0, 320), simplify = F)
titles <- list("$\\sigma^2_e = 1.00; \\phi = 0.3$", 
               "$\\sigma^2_e = 0.33; \\phi = 0.3$",
               "$\\sigma^2_e = 1.00; \\phi = 0.7$",
               "$\\sigma^2_e = 0.33; \\phi = 0.7$")
p <- p_log <- list()
p <- Map(plot_expected_RTs, s2_e = c(1, 1/3, 1, 1/3), phi = c(0.3, 0.3, 0.7, 0.7), 
         func = "dnorm", lims = lim_RT, title = titles, xlabel = "log(RT)")
         # func = "dnorm", lims = lim_RT, title = "\u03B1 = 1; \u03D5 = 0.3", xlabel = "log(RT)")
p_log <- Map(plot_expected_RTs, s2_e = c(1, 1/3, 1, 1/3), phi = c(0.3, 0.3, 0.7, 0.7), 
             func = "dlnorm", lims = lim_logRT, title = titles, xlabel = "RT")

p_log[[1]]

grid.arrange(p[[1]], p[[2]], p[[3]], p[[4]], nrow = 2)
grid.arrange(p_log[[1]], p_log[[2]], p_log[[3]], p_log[[4]], nrow = 2)

############# save both plots
discri_comp_logRT <- arrangeGrob(p[[1]], p[[2]], p[[3]], p[[4]], nrow = 2)
discri_comp_RT <- arrangeGrob(p_log[[1]], p_log[[2]], p_log[[3]], p_log[[4]], nrow = 2)
# ggsave("discrimination_comp.eps", discri_comp, width = 16, height = 8, device = "eps")

cairo_ps("discrimination_comp_logRT.eps", width = 16, height = 12, fallback_resolution = 1200)
grid:::grid.draw(discri_comp_logRT)
dev.off()

cairo_ps("discrimination_comp_RT.eps", width = 16, height = 12, fallback_resolution = 1200)
grid:::grid.draw(discri_comp_RT)
dev.off()
