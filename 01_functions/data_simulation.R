## Packages
############################################################## 
library(MASS)

## Meta-Function (User-Interface)
############################################################## 
simulateData <- function(n, mu_abil, sd_abil, 
                         speedValues, 
                         timeLimit,
                         n_items, mu_I, Sigma_I,
                         mu_varEpsilon, var_varEpsilon, mu_alpha, var_alpha,
                         model, discr_shift) {

  ################################## Person parameters 
  # per test version
  # two dimensions (ability and speed), uncorrelated
  
  ### mvn version
  # s <- matrix(c(sd_abil, 0, 0, sd_speed), ncol = 2, nrow = 2)
  # pers_drawn <- mvrnorm(n = n, mu = c(mu_abil, mu_speed), Sigma = s)
  
  
  ### categorical version
  pers_drawn <- data.frame(abil = rnorm(n = n/4, mean = mu_abil, sd = sd_abil),
                           speed = unlist(lapply(speedValues, function(value) rep(value, n/4))))
  # categorize speed
  speed_cats <- cut(pers_drawn[, 2], breaks = quantile(pers_drawn[, 2]), include.lowest = TRUE,
  labels = c("slowest", "slow", "fast", "fastest"))

  pers <- data.frame(abil = pers_drawn[, 1],
                     speed = pers_drawn[, 2],
                     speedCat = speed_cats)
  
  ################################## Item Parameters
  # generate a testform (low discriminating) note: alpha is reversed in the Hierarchical RT model!
  # 30 items (2pl for ability, 2pl for speed), but all discriminations equal
  # parameters chosen in accordance to van der Linden (2006)
  items <- vector("list")
  
  # prevent negative discrimination parameters
  items$lowDis <- data.frame(a = -1)
  while(any(items$lowDis$a < 0) || any(items$lowDis$phi < 0)) {
    items$lowDis <- data.frame(mvrnorm(n_items, mu = mu_I, Sigma = Sigma_I, empirical = TRUE))
    names(items$lowDis) <- c("b", "a", "lambda", "phi")
  }
  
  ### set discrimination and error variance in dependece of model
  if(identical(model, "vanderLinden") && missing(var_varEpsilon) && missing(mu_phi) && missing(sd_phi)) {
   # discrimination is 1 for all items in van der Linden
    items$lowDis$phi <- rep(1, n_items)
    # Alpha is parameter for residual variance
    items$lowDis$alpha <- mvrnorm(n_items, mu_alpha, Sigma = var_alpha, empirical = TRUE) 
   # can be transformed
   items$lowDis$var_res <- sqrt(items$lowDis$alpha^(-2)) 
  } else if(identical(model, "Fox") && missing(mu_alpha) && missing(var_alpha)) {
    # generate itemwise residual variances
    items$lowDis$var_res <- -1
    while(any(items$lowDis$var_res < 0)) {
      items$lowDis$var_res <- mvrnorm(n_items, mu = mu_varEpsilon, Sigma = var_varEpsilon, empirical = TRUE) 
    }
  } else stop("Wrong parameters specified")    
  
  ### second testform, identical parameters
  stopifnot(length(names(discr_shift)) == length(discr_shift))
  
  for(n in names(discr_shift)) {
    newName <- paste0(n, "Dis")
    items[[newName]] <- items[[1]]
    if(model == "vanderLinden"){
      stop("This method is no longer supported.")
      items$highDis$alpha <- items[[1]]$alpha + discr_shift
      items$highDis$var_res <- sqrt(items$highDis$alpha^(-2))
      # correct time intensity for similar average response times
      items$highDis$lambda <- items[[1]]$lambda + discr_shift/4 ### different correction or as additional argument?
    } else if(model == "Fox"){
      items[[newName]]$phi <- items[[1]]$phi + discr_shift[n]
    }
  }
  
  ################################## Simulate Responses
  # according to van der Linden RT model (responses and response times)
  ### responses (for both test forms)
  logits <- lapply(items, function(i) irtLogits(theta = pers$abil, a = i$a, b = i$b))
  probits <- lapply(logits, logit2prob)
  Ytrue <- lapply(probits, drawResponseDF)
  
  ### testing
  #browser()
  #test <- fixedIRT(dat = Ytrue$lowDis[1:500, ], a = items$lowDis$a, b = items$lowDis$b)
  #test <- tam.mml.2pl(Ytrue$lowDis[1:500, ], irtmodel = "2PL")
  #mean(pers$abil[1:500] - test$person$EAP[1:500])
  #sd(pers$abil[1:500] - test$person$EAP[1:500])
  #hist(pers$abil[1:500] - test$person$EAP[1:500])
  #test$wle <- tam.wle(test)
  #mean(test$person$EAP[1:500])
  #mean(test$wle$theta[1:500])
  #mean(pers$abil[1:500] - test$wle$theta[1:500])
  
  
  ################################## Simulate Responses Times
  ### response times
  rtData <- lapply(items, function(i) doRTs(zeta = pers$speed, 
                                            lambda = i$lambda, phi = i$phi, var_E = i$var_res))
  
  ################################## Insert NAs
  # insert NAs for not reached items (set time Limit!) and score them as incorrect
  Yreal <- Map(function(Y, RT) generateNAs(Ytrue = Y, cumRT = RT$cumResponseTimes, timeLimit = timeLimit), 
               Y = Ytrue, RT = rtData)
  
  #### output
  list(resp = Yreal, respTimes = rtData, complResp = Ytrue, itemPar = items, persPar = pers)
}



## 0. Common Functions
############################################################## 
###### logit and probit conversion
logit2prob <- function(logit){
  odds <- exp(logit)
  prob <- odds / (1 + odds)
  return(prob)
}

prob2logit <- function(probit){
  logit <- log(probit/(1-probit))
  logit
}


## 1. Response Simulation Functions
############################################################## 
## 2pl model, output logits in matrix
irtLogits <- function(theta, a, b) {
  stopifnot(length(a) == length(b))
  stopifnot(is.double(theta))
  
  logits <- matrix(NA, nrow = length(theta), ncol = length(b))
  for(p in seq_along(theta)) {
    logits[p, ] <- a * theta[p] - b ## 2PL Modell; carefull with formula: before a * (theta - b) was used, but tams seems to use other one!
  }
  logits
}

## draw responses for probits; via comparing to random number between 0 and 1
drawResponseDF <- function(probDF) {
  stopifnot(is.matrix(probDF))
  
  responses <- randoms <- probDF
  randoms <- runif(nrow(probDF) * ncol(probDF), min = 0, max = 1)
  responses <- ifelse(probDF > randoms, 1, 0)
  responses
}


## 2. Simulate Response times 
############################################################## 
#### META-Function
doRTs <- function(zeta, lambda, phi, var_E) {
  rt <- drawRTDF(zeta = zeta, lambda = lambda, phi = phi, var_E = var_E)
  
  ## accumulate response times
  cumRT <- rowapply(rt, cumsum)
  list(responseTimes = rt, cumResponseTimes = cumRT)
}


## lognormal response time model
drawRTDF <- function(zeta, lambda, phi, var_E) {
  stopifnot(length(lambda) == length(phi) && length(lambda) == length(var_E))
  stopifnot(is.double(zeta))
  
  #browser()
  E <- matrix(NA, nrow = length(zeta), ncol = length(lambda))
  for(i in seq_along(lambda)) {
    E[, i] <- mvrnorm(nrow(E), mu = 0, Sigma = var_E[i, ]) # see below
  }
  
  logs <- matrix(NA, nrow = length(zeta), ncol = length(lambda))
  for(p in seq_along(zeta)) {
    logs[p, ] <- exp(lambda - (phi * zeta[p]) + E[p, ])   ## Fox & Marianti (2017), van der Linden with phi = 1 for all i 
  }
  logs
}

## 
rowapply <- function(dat, FUN) {
  stopifnot(is.data.frame(dat) | is.matrix(dat))
  
  out <- dat
  for(r in seq(nrow(out))) {
    out[r, ] <- FUN(dat[r, ])
  }
  out
}

## 3. Inserting NAs 
############################################################## 
generateNAs <- function(Ytrue, cumRT, timeLimit) {
  #cut-of values
  cutRT <- timeLimit ## seconds
  D <- ifelse(cumRT > cutRT, 1, 0)
  
  # code not reached as NAs
  Yreal <- ifelse(D == 1, NA, Ytrue)
  
  out <- list(unscoredData = Yreal)
  out
}

## 4. check resulting data for items with no correct answers
############################################################## 
check0 <- function(datList) {
  check_list <- lapply(datList, function(x) {
    apply(x$resp$highDis$unscoredData, 2, function(var) {
      
      var[is.na(var)]  <- 0                                       
      all(var == 0)
    })
  })
  
  which(unlist(lapply(check_list, any)))
}


